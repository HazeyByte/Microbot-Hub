...
import net.runelite.client.plugins.microbot.irkedSlayer.PrayerFlickStyle;
import net.runelite.client.plugins.microbot.irkedSlayer.IrkedSlayerConfig;
import net.runelite.client.plugins.microbot.irkedSlayer.IrkedSlayerPrayer;
...
@Slf4j
@Singleton
public class IrkedSlayerFlickerScript {
    ...
    public static final AtomicReference<List<IrkedSlayerMonster>> currentMonstersAttackingUsRef = new AtomicReference<>(new ArrayList<>());
    private final AtomicReference<List<Rs2NpcModel>> npcsRef = new AtomicReference<>(new ArrayList<>());

    private IrkedSlayerConfig config;
    private IrkedSlayerPrayer activePrayer = null;
    private int tickToFlick = 0;
    private boolean isRunning = false;

    public void start(IrkedSlayerConfig config) {
        this.config = config;
        this.isRunning = true;
        log.info("IrkedSlayerFlickerScript started");
    }
    ...
    public void stop() {
        ...
        activePrayer = null;
        log.info("IrkedSlayerFlickerScript stopped");
    }

    /**
     * Sets the active prayer for flicking
     */
    public void setActivePrayer(IrkedSlayerPrayer prayer) {
    ...
    public void onGameTick() {
        ...
        currentMonstersAttackingUsRef.updateAndGet(monsters -> {
            List<IrkedSlayerMonster> updated = new ArrayList<>(monsters);
        ...
        for (IrkedSlayerMonster monster : currentMonstersAttackingUsRef.get()) {
            monster.lastAttack--;
     ...
     */
    private Rs2PrayerEnum getPrayerToActivate(IrkedSlayerMonster monster) {
        if (activePrayer != null && activePrayer != IrkedSlayerPrayer.NONE) {
    ...
    public void resetLastAttack(boolean forceReset) {
        ...
        currentMonstersAttackingUsRef.updateAndGet(monsters -> {
            ...
            for (Rs2NpcModel npc : npcs) {
                IrkedSlayerMonster m = updated.stream()
                ...
                if (m != null) {
                    ...
                    IrkedSlayerMonster toAdd = new IrkedSlayerMonster(npc, stats);
