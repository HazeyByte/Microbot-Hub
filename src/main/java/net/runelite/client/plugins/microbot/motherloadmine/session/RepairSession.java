...
public class RepairSession extends Session {
    ...
    public RepairSession(Rs2TileObjectCache tileCache) {
    ...
    private void tickVerify() {
        ...
        if (brokenBeforeClick >= 0 && broken < brokenBeforeClick) {
            ...
            if (broken > 0 && Rs2Random.between(0, 100) < 70) {
    ...
    private void tickCleanup() {
        // 20% chance to pause 1-2 seconds after repair (admiring work)
        if (Rs2Random.between(0, 100) < 20 && !postRepairPaused) {
    ...
    private void selectTargetStrut() {
        ...
        // Shuffle for genuine randomness — toList() order is deterministic
        java.util.Collections.shuffle(struts, new java.util.Random(System.nanoTime()));

        var chosen = struts.get(0);
    ...
    private boolean isAnotherPlayerRepairing() {
        ...
        WorldPoint strutArea = new WorldPoint(3747, 5672, 0);
