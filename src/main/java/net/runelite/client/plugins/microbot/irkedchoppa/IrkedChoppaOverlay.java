...
public class IrkedChoppaOverlay extends OverlayPanel {
    ...
    private final IrkedChoppaConfig config;
    ...
    @Inject
    IrkedChoppaOverlay(IrkedChoppaPlugin plugin, IrkedChoppaConfig config, Client client) {
        super(plugin);
        this.plugin = plugin;
        this.config = config;
        this.client = client;
    ...
    public Dimension render(Graphics2D graphics) {
        ...
        try {
            ...
            panelComponent.getChildren().add(TitleComponent.builder()
                    .text("irkedChoppa v" + IrkedChoppaPlugin.version)
                    .color(TITLE_COLOR)
                    .build());
            ...
            panelComponent.getChildren().add(LineComponent.builder()
                    .left("Status:")
                    .right(this.plugin.irkedChoppaScript.irkedChoppaScriptState.toString())
            ...
            // Current tree
            IrkedChoppaTree tree = plugin.getSelectedTree();
            if (tree != null) {
                panelComponent.getChildren().add(LineComponent.builder()
                        .left("Tree:")
                        .right(tree.name())
            ...
            panelComponent.getChildren().add(LineComponent.builder()
                    .left("Action:")
                    .right(config.action().toString())
                    .rightColor(NORMAL_TEXT_COLOR)
                    .build());

            // Display area boundary
            if (config.distanceToStray() < 21) {
                LocalPoint lp = LocalPoint.fromWorld(client, IrkedChoppaScript.getReturnPoint(config));
                if (lp != null) {
