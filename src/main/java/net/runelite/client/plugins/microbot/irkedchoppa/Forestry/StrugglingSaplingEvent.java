...
import net.runelite.client.plugins.microbot.irkedchoppa.IrkedChoppaScript;
...
public class StrugglingSaplingEvent implements BlockingEvent {
    ...
    public StrugglingSaplingEvent(IrkedChoppaPlugin plugin) {
        this.plugin = plugin;
    }
        ...
        try{
            ...
            return strugglingSaplings.stream().anyMatch(obj ->
                    Rs2GameObject.hasAction(Rs2GameObject.convertToObjectComposition(obj), "Add-mulch") &&
                            obj.getWorldLocation().distanceTo(Rs2Player.getWorldLocation()) <= IrkedChoppaScript.FORESTRY_DISTANCE
            );
        } catch (Exception e) {
