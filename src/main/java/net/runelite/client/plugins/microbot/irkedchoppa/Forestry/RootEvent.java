package net.runelite.client.plugins.microbot.irkedchoppa.Forestry;
...
import net.runelite.client.plugins.microbot.irkedchoppa.IrkedChoppaPlugin;
...
public class RootEvent implements BlockingEvent {
    ...
    public RootEvent(IrkedChoppaPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean validate() {
        try{
            ...
            var root = plugin.getRs2TileObjectCache().query().where(x -> x.getId() == ObjectID.GATHERING_EVENT_RISING_ROOTS).nearest();
            var specialRoot = plugin.getRs2TileObjectCache().query().where(x -> x.getId() == ObjectID.GATHERING_EVENT_RISING_ROOTS_SPECIAL).nearest();
